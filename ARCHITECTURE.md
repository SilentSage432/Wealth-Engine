...
lib/babylon/engine.ts                       → pure 10/20/70 allocation math
lib/babylon/persistence.ts                  → localStorage load/save/clear
lib/babylon/constants.ts                    → rates, wisdom, nav, labels, EMPTY_STATE
components/babylon/*                        → presentation zones (sidebar, triad, charts, ledgers, dialog)
...
Gross Income
    ...
    └─ 70% → Expenditure Allowance
              └─ Expenses (Need | Desire) draw down remaining
```
...
- `RecordTributeDialog` owns ephemeral form fields; the hook owns durable ledger mutations (`addIncome` / `addExpense` / `addDebt` / `clearAllData`).

## Persistence contract
Key: `wealth-engine-babylon-v2`  
Shape: `{ incomes, expenses, debts, allocations, displayName }`  
Cold start: empty arrays when no stored payload exists.
