...
- [x] Notification persistence foundation (WE-NOTIFY-002). A per-user preference stores an explicit enabled flag and an IANA timezone. Push subscription rows store an endpoint and its keys for that authenticated user. Owner RLS applies. No permission prompt, no browser subscription, no Web Push send, no scheduler, no delivery history, and no vault change. Notifications are not live. `20260929_notification_foundation.sql` is written and not applied from the app
- [ ] Multi-currency
