...
- Phone shell: below `lg` (1024px), only the phone tree mounts. One local destination owns Home, Budget, Ledger, and More (`mobileDestination` in `wealth-engine-dashboard.tsx`). It is not synced with desktop `activeNav`. The bottom bar is `components/babylon/mobile-bottom-nav.tsx`. The compact header is `components/babylon/mobile-header.tsx`. Phone Home (`components/babylon/mobile-home.tsx`) is attention, Money Available, Protected Money, Available After Planned Needs, a three-item upcoming preview, and a short recent slice. Due actions stay Paid / Still upcoming. Last-day month-close Attention stays in the header. Phone Budget (`components/babylon/mobile-budget.tsx`) is Living Budget, the 10/20/70 purpose, categories, debt payoff, then income, charts, and affordability only after the steward opens them. Phone Ledger (`components/babylon/mobile-ledger.tsx`) is stacked Income, Expenses, and Debts records with the existing paid mark, occurrence edit, and monthly-rule edit. Desktop Ledger keeps `LedgerMatrices` tables. Phone More (`components/babylon/mobile-more.tsx`) groups Financial setup, Connections, Guidance, Data and cloud, and Danger zone. It reuses the vault maintenance parts. The desktop sidebar still mounts `VaultMaintenancePanel` as a whole. More holds account management, profile editing, Close Month, bank connection, Financial Guidance, and vault maintenance. At `lg` and above only the desktop tree mounts (`hooks/useDesktopLayout.ts`): sidebar, `CommandBar`, and `SpeedTributeBar`. Those sticky surfaces stay opaque slate
...
- Notification persistence (WE-NOTIFY-002): `notification_preferences` and `push_subscriptions` in `supabase/migrations/20260929_notification_foundation.sql`. Authenticated routes under `app/api/notifications/*` read and write those rows for the session user only. This does not request permission, create a browser subscription, display or send a notification, evaluate Attention while closed, or schedule work. Notifications are not live. The migration is written and not applied from the app. These rows are not part of the local vault or revision sync
...
- Notification preference and push subscriptions are not in this document. They are cloud rows (`notification_preferences`, `push_subscriptions`) outside revision sync. WE-NOTIFY-002 stores them only. It does not send or schedule notifications.
